clc, clear, close all
%% Problem 1 Part 3: Formulate a linear program with Maximum 25% Invested in B-rated Bonds
%% Given information
...
B_rated_limits = [
    3*108, 3*94, 3*99, 3*92.7, 3*96.6, 3*95.9,...  % Positive on all B-rated bonds, for max 25% B-rated bonds, 3*B-rated bonds <= 1*A-rated bonds
    -92.9, -110, -104, -101, -107, -102, -95.2,... % Negative on all A-rated bonds
    0, 0, 0, 0, 0                                  % No cost for reinvesting
    ];
B_rated_inequality = 0;                            % 3*B-rated bond value - A-rated bond value <= 0
